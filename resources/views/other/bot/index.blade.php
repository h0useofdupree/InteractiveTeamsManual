...
@section('content')
    ...
    <script type="text/javascript" {{-- src="{{ asset('js/chatbot.js') }}" --}}>
        /******/
        (() => { // webpackBootstrap
            ...
            function send_message(message) {
                var timeout = setTimeout(function() {
                    ...
                    div.innerHTML +=
                        '</img><div id="chatb"><img src="data/bot-1.png" id="pimg" onmousedown="return false"><a id="title">' +
                        namebot + '</a><br><br>' + message + '<br><a id="date"><br>' + datetime() +
                        '</a></div><span id="space"></span>';
                    ...
                    fetch("json/files.json").then(function(response) {
                        ...
                        fetch("json/filter.json").then(function(response) {
                            ...
                            for (var o = 0; o < filter['filter'].length; o++) {
                                string = string.replace(filter['filter'][o]['string'] + "/g", filter[
                                    'filter'][o]['re']);
                                console.log(filter['filter'][o]['string'] + " - " + filter['filter'][o][
                                    're'
                                ]);
                            }
                            ...
                            for (var v = 0; v <= data['article'][u]['keywords'].length; v++) {
                                if (i.indexOf(data['article'][u]['keywords'][v]) >= 0) {
                                    for (var x = 0; x < data['article'][u]['pages'].length; x++) {
                                        if (linkvar == true) {
                                            writingelement =
                                                'Informationen zu deinem gewünschtem Thema findest du hier: <a id="links" href="' +
                                                data['article'][u]['link'] + '">' + data['article'][u][
                                                'header'] + '</a>';
                                            b = data['article'][u]['link'];
                                        } else {
                                            writingelement += "<b>" + [x + 1] + ".</b> " + data['article'][u][
                                                'pages'
                                            ][x]['content'] + " <br>";
                                        }
                                    }
                        ...
                        fetch("json/conv.json").then(function(response) {
                            ...
                            if (writingelement == "") {
                                ...
                                switch (rdm) {
                                    case 0:
                                        writingelement =
                                            "Das wonach du suchst konnte ich leider nicht finden. Probiere es vielleicht mit anderen Schlagwörtern";
                                        break;
                                    ...
                                    case 2:
                                        writingelement =
                                            "Error: Ich habe leider nicht verstanden was du von mir möchtest!";
                                        break;
                                }
                            } //* Easter Eggs mit Spezialfunktionen


                            if (i.indexOf("HINDI") >= 0 || i.indexOf("INDER") >= 0 || i.indexOf(
                                    "INDIEN") >= 0 || i.indexOf("INDISCH") >= 0 || i.indexOf("INDIA") >=
                                0 && ttsvar == true) {
                                hindi = true;
                                namebot = "Shiva";
                            }
                            ...
                            if (hindi == true) {
                                var timeout = setTimeout(function() {
                                    ...
                                    if (r == 0) {
                                        ...
                                        namebot = "Jenny";
                                        send_message(
                                            "Ehm. Wer war das denn jetzt... Also ich war das nicht. Du musst mir das glauben."
                                            );
                                    }
                                }, hindi_ee_delay);
                            }
            ...
            $(function() {
                ...
                send_message(
                    "Hallo mein Name ist Jenny. Ich helfe dir gerne bei allen Problemen bezüglich Microsoft Teams. Was ist dein Anliegen?<br> Notiz: Ich bin nicht sehr schlau :)"
                    );
                ...
                $("#send").click(function() {
                    ...
                    if (message != "") {
                        var div = document.getElementById('container');
                        div.innerHTML +=
                            '<div id="chatu"><img src="data/user-1.png" id="pimg" onmousedown="return false">' +
                            user + message + '<br><a id="date"><br>' + datetime() + '</a>' +
                            '</div><br><br>';
                    }
